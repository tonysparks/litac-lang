...
| Migrations | `std/db/migrations/migrations` | Versioned schema migrations |
...
The ORM generates a `getSqlCreateTable` method you can call once at startup:

```lita
var sb = StringBuilderInit(1024, defaultAllocator)
var p = Product{}
p.getSqlCreateTable(&db, &sb)
db.execute(sb.cStr())
sb.free()
```
...
```lita
import "std/db/migrations/migrations"
