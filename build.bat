...
java -jar %LITAC_HOME%/target/litac.jar -profile -buildCmd %BUILD_CMD% -outputDir "./bin" -output "litaC" "./src/main.lita" -types "none" -debug
cd bin
echo Running litaC...
REM litaC.exe -debug -profile -buildCmd %BUILD_CMD% "../src/main.lita"
litaC.exe -cFormat -types all -debug -run -profile "../test/test_single.lita"
